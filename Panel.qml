...
Panel {
  ...
  // ---------- Session actions: Lock / Sleep / Restart / Shutdown / Kill ----------
  // Each closes the panel first since the action is either about to obscure
  // the screen (lock, sleep) or tear the session down (restart, shutdown,
  // kill) -- there is nothing left for an open panel to usefully show.
  function lockSession() {
    root.close()
    lockProc.command = ["omarchy-system-lock"]
  ...
  function shutdownSession() {
    root.close()
    shutdownProc.command = ["omarchy-system-shutdown"]
    shutdownProc.running = true
  }

  // Immediate, unconfirmed hard power-off -- no orderly shutdown, nothing
  // synced or unmounted first. `systemctl poweroff --force --force` calls
  // the power-off syscall directly rather than going through PID1, which
  // needs CAP_SYS_BOOT -- hence pkexec: this is a GUI button with no
  // terminal to type a sudo password into.
  function killSession() {
    root.close()
    killProc.command = ["pkexec", "systemctl", "poweroff", "--force", "--force"]
  ...
  // Fire-and-forget: one Process per session action so rapid double-clicks
  // across different buttons can't collide on a shared `running` flag.
  Process { id: lockProc }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      Column {
        ...
        // ---------- Session actions: Lock / Sleep / Restart / Shutdown / Kill ----------
        // Icon-only row, name on hover (Button auto-renders tooltipText).
        // Mouse-only: not wired into the panel's j/k/Enter cursor model.
        PanelSeparator {
          foreground: root.bar.foreground
        }
